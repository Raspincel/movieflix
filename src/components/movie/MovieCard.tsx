...
const MovieCard = ({ movie }: Props) => {
  return (
    <Link to={`/filme/${movie.id}`}>
      <div
        key={movie.id}
        className="relative group overflow-hidden rounded-2xl shadow-xl transition-all duration-500 hover:shadow-cyan-500/20 hover:shadow-xl max-w-96 aspect-[500/750] mx-2 cursor-pointer"
      >
        {movie.cover ? (
          <img
            src={movie.cover}
